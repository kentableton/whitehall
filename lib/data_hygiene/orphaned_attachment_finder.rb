...
module DataHygiene
  class OrphanedAttachmentFinder
    ...
    def summarize_by_type
      summary = "Documents with all/some attachments not referenced by inline tags:\n\n" +
        ("%-30s %-11s %-11s" % ["", "Published", "Other"]) + "\n" +
        ("%-30s %5s %5s %5s %5s" % ["", "All--", "Some-", "All--", "Some-"]) + "\n"


      other_types = editions_with_orphaned_attachments.group_by {|e| e[:edition].class.name }.map do |class_name, batch|
